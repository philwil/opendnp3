...
#include <opendnp3/outstation/MyCommandHandler.h>
...
int main(int argc, char* argv[])
{
    ...
    MyOutputs.push_back(21);
    MyOutputs.push_back(22);
    MyOutputs.push_back(23);
    const auto commandHandler = std::make_shared<MyCommandHandler>(MyOutputs);
...
void AddUpdates(UpdateBuilder& builder, State& state, const std::string& arguments)
{
    for (const char& c : arguments)
    {
        ...
        case ('A'):
        {
            builder.Update(Analog(state.value), 1);
            state.value += 1;
            break;
        }
